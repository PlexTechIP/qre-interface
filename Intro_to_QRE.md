...
The Microsoft Quantum resource estimator is an [open-source](https://learn.microsoft.com/en-us/azure/quantum/intro-to-resource-estimation) tool that allows you to estimate the resources needed to run a quantum program on a fault-tolerant quantum computer.
...
| Level | Component |
|---|---|
| Application | High-level quantum program (Q#, Qiskit) |
| Compilation | Quantum Intermediate Representation |
| Compilation | Quantum ISA (T factory distillation) |
| Compilation | Microcode (QEC) |
| Modeling | Microarchitecture (Physical qubit parameters) |
...
You can use pre-defined qubit parameters and quantum error correction (QEC) schemes or configure unique settings across a wide range of machine characteristics. For more information, see Customize the target parameters of the resource estimator.
...
| Quantum error correction scheme | For example, specify the number of physical qubits per logical qubit, the logical cycle time, or the error correction threshold. |
...
> **Note**
> With the resource estimator, you can model any quantum architecture. For example, the startup Alice & Bob's uses the resource estimator to evaluate their architecture, which uses cat qubits and repetition error correction code. For more information, see this post in the Q# blog.

## Flexibility

You can bring your own code and compilation tools to the resource estimator. The resource estimator supports Q# and Qiskit as input. See Different ways to run the resource estimator.

## Batch multiple estimates

The resource estimator allows you to estimate the resources needed to run the same quantum algorithm for different configurations of target parameters, and compare the results. In this way you can understand how the qubit architecture, QEC scheme, and the rest of the target parameters impact the overall resources.

## Optimization

You can reduce the execution time of the resource estimator by incorporating some estimates in the overall cost. For example, if you're working with a large program, you can compute and cache the cost of subroutines, or if you already know estimates for an operation you can pass them to the resource estimator.

## Visualization of resources

You can visualize the tradeoffs between the number of physical qubits and the runtime of the algorithm using the space-time diagram, which allows you to find the optimal combination of {number of qubits, runtime} pairs.

You can also inspect the distribution of physical qubits used for the algorithm and the T factories using the space diagram.

## Get started with the resource estimator

The resource estimator is part of the Quantum Development Kit (QDK) and doesn't require an Azure account. To get started, see Run your first resource estimate.
...
|---|---|
| I'm developing QEC codes | You can use the resource estimator to customize your QEC codes and compare different combinations of parameters. See How to customize your QEC schemes. |
| I'm developing quantum algorithms | By analyzing the impact of different configurations of hardware and software profiles on the resource requirements, you can gain insights into how your quantum algorithm might perform under different hardware and error conditions. This information can help you optimize your algorithm for specific quantum hardware or error rates. See Running multiple configurations of target parameters. |
| I want to improve the performance of quantum programs | To learn how to harness the power of the resource estimator, see Running large programs and Using known estimates. |
| I'm interested in large-scale quantum computing | You can use the resource estimator to analyze the resources of real-world problems that are expected to be solved by large-scale fault-tolerant quantum computers. See how in Resource estimation for large-scale quantum computing. |
| I'm developing quantum-safe cryptography | You can use the resource estimator to compare the performance of different encryption algorithms, key strengths, qubit types, and error rates, and their resilience to quantum attacks. See Resource estimation and cryptography. |

If you want to develop quantum algorithms for large-scale quantum computers, check out the Estimate the resources of a quantum chemistry problem tutorial.
...
*Source: [Introduction to the resource estimator - Azure Quantum | Microsoft Learn](https://learn.microsoft.com/en-us/azure/quantum/intro-to-resource-estimation)*
