...
| **PSSPC** | Pauli-based trace transform whose knobs are **T Count Per Rotation** (5–20, default 20 — T states spent synthesizing each arbitrary rotation) and **CCX magic states** (whether CCX/Toffoli gates consume dedicated magic states; default false). |
| **Total Fault Tolerant Execution Error** | Configuration input: the cap on total logical error probability, in (0, 1], default 1.0 (= unconstrained). It bounds the result's **total error** rather than being split up front. |
