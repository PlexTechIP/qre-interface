...
| **5** | **Fri Jul 31 → Wed Aug 5** | **Published.** Re-scoped after the Fri Jul 31 POC. T3 = the configuration surface end to end (POC tooltips + terminology, one merged factory control, the **four-stage trace pipeline**, four new QPU fields, contract wiring + engine adapter); T1 = **MCP server research**, documentation only; T2 = **the LLM interface** (bring-your-own API key, natural language → draft config). PMs land contract **v1.4.0** at kickoff | Part 1 + 2 polish; agentic stretch goal opens | Short week: Day 0 Fri Jul 31, checkpoint meeting Tue Aug 4, **deadline Wed Aug 5 EOD**. Check-in/POC Fri 7/31 produced the field-level UI action list. |
| **6** | Wed Aug 5 → Tue Aug 11 | **Packaging** (electron-builder, the Python bundling decision, cross-platform matrix, signing once Microsoft answers); pull-oriented update mechanism; **carried from week 5: Part 3 export hardening** (native save dialog, full-field output, version-tracking enforcement) and the **program library / persisted uploads** | Part 3 → Part 4 | Check-in Fri 8/7. Packaging risk is the summer's largest open item — the ~428 MB Python venv drives it. |
