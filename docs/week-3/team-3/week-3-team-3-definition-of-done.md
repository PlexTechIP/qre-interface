...
- [x] **Electron main process stood up** in `app/src/main/`: app lifecycle, a
...
- [x] **Typed IPC bridge** exposes an **`EstimatorService`-shaped** API to the
...
- [x] **`MockEngine → QreEngine` swapped** behind that interface: the app runs
      the real engine by default; the MockEngine remains as the reference/test
      double
- [x] **Real QRE runs via the chosen route** (`qdk[qre]==1.29.1` on Python
...
- [x] **Python setup/bundling** documented and reproducible on macOS **and**
...
- [x] `RunConfig` (renderer→main) and `RunResult` (main→renderer) cross intact
      (structured-clone-safe); **failures cross as resolved `RunResult`s**
      (`status: "failed"`), never as thrown/hanging IPC errors
- [x] **Conformance harness green through the IPC path** on every committed
...
- [x] **Timeout enforced end-to-end**; the subprocess is cleaned up on
      completion, timeout, and app quit (no orphaned processes); two sequential
      runs don't interfere
- [x] **Teams 1 & 2's surfaces verified unregressed** by the swap (Run
      Configuration + Results behave as they did on the mock)
- [x] **Carry-over from week 2:** the capture spread is finished (a `litinski19`
      capture + a 2nd failure class) and the fragile substring `COMPILE_ERROR`
      classifier is replaced with a robust check

## Quality

- [x] Strict TypeScript; no `any` at the IPC boundary; the renderer imports only
      the `EstimatorService` surface — no engine/subprocess imports
      (grep-provable)
- [x] Only the whitelisted `EstimatorService` methods are exposed over IPC (no
      general `ipcRenderer` handle in the renderer)
- [x] No hardcoded absolute paths anywhere; the Python runtime resolves relative
      to the app
- [x] Engine-module **README + known-issues handoff** updated for next week's
